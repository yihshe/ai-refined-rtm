...
class SetupMultiple:
    ...
    def __init__(self, ns, paras, depends):
        # list of all parameters with a logical distribution (start, stop, steps)
        self.whichlogicals = []
        self.nruns_logic_geo, self.nruns_logic_no_geo, self.nruns_logic_total = (
            1, 1, 1)
        ...
        # how many parameters are part of the LUT? Notice that these are not the
        ...
        # paras are input of this class as a dictionary (**kwargs)
        self.paras = paras
        # ns: number of statistical distributions; i.e. "draw #ns times from the distribution"
        self.ns = int(ns)
        self.error_array = []
    ...
    def create_grid(self):
        ...
        for para_key in self.paras:
            if len(self.paras[para_key]) == 3:  # logical distr. = min, max, nsteps
                # append parameter to the list of logically distributed paras
        ...
        for para_key in self.whichlogicals:
            if para_key in ["tts", "tto", "psi"]:
                ...
                self.nruns_logic_geo *= self.nruns_logic[para_key]
            else:
                # contains everything except geometry
                self.nruns_logic_no_geo *= self.nruns_logic[para_key]
        self.nruns_logic_total = self.nruns_logic_geo * self.nruns_logic_no_geo

        # 2 Calculate total nruns
        # ns draws from distribution * logical steps in total
        self.nruns_total = int(self.nruns_logic_total * self.ns)

        # 3 Create numpy array to hold all parameter constellations
        self.para_grid = np.empty(
            shape=(self.nruns_total, self.npara), dtype=np.float32)

        # 4 Fill each parameter into numpy array
        k = 0  # iterator for logical order
        # initialize value for "accumulated repetition of variable"
        ...
        for para_key in self.paras:
            if len(self.paras[para_key]) == 2:  # uniform distribution
                ...
                self.para_grid[:, self.para_nums[para_key]] = np.tile(
                    self.paras[para_key][0], self.nruns_total)

            elif len(self.paras[para_key]) == 3:  # logically distributed parameter
                k += 1
                # how often do all ns-distributed parameters need to be
            ...
            if self.depends == 1 and para_key == 'car':
                self.para_grid[:, self.para_nums[para_key]] = \
                    self.car_cab_dependency(
                        grid=self.para_grid[:, self.para_nums['cab']])  # set car according to cab
    ...
    def logical_distribution(self, para_name, min, max, repeat, multiply, nsteps):
        ...
        return_list = np.tile(np.repeat(np.linspace(
            start=min, stop=max, num=int(nsteps)), repeat), multiply)
        return return_list
    ...
    def gauss_distribution(self, para_name, min, max, mean, sigma, multiply):
        ...
        try:
            ...
            raise ValueError(
                "Cannot create gaussian distribution for parameter {}}. Check values!".format(para_name))
        return return_list

    def uniform_distribution(self, para_name, min, max, multiply):
        try:
            return_list = np.tile(np.random.uniform(
                low=min, high=max, size=self.ns), multiply)
        except:
            raise ValueError(
                "Cannot create uniform distribution for parameter %s. Check values!" % para_name)
        return return_list
    ...
    def car_cab_dependency(self, grid):
        ...
        car_lin_noise = np.random.laplace(
            loc=0, scale=spread, size=len(car_lin)) + car_lin
        car_lin_noise = refine_noise(
            car_lin, car_lin_noise, lower_car, upper_car)
        car_lin_noise = refine_noise(
            car_lin, car_lin_noise, lower_car, upper=np.tile(26, len(grid)))
...
class InitModel:
    ...
    def initialize_vectorized(self, LUT_dir, LUT_name, ns, max_per_file=5000, soil=None,
        ...
        if len(paras["tts"]) > 1 or len(paras["tto"]) > 1 or len(paras["psi"]) > 1:
            # LUT-Files are (firstly) sorted by geometry
            self.geo_mode = "sort"
        else:
            self.geo_mode = "no_geo"

        # defines the number of PROSAIL runs in one file ("split"), default=5000
        self.max_filelength = max_per_file
        # how many parameters are stored in the LUT
        npara = len(self.para_names)
        # (at maximum! Does NOT depend on the version of PROSPECT / SAIL used)
        # Prepare for setting up PROSAIL
        ...
        # The number of PROSAIL executions
        ...
        if crun_pergeo <= max_per_file:  # exactly one LUT-file per Geo
            ...
            # number of ensembles
            n_ensembles_split = (crun_pergeo - 1) // max_per_file + 1
        ...
        if testmode == True:
            start = time.time()
            # simplistic form of running PROSAIL
        ...
        with open("%s_00meta.lut" % (LUT_dir + LUT_name), "w") as meta:
            ...
            meta.write("\nparameters={}".format(
                ";".join(i for i in self.para_names)))
            meta.write("\nwavelengths={}".format(
                ";".join(str(i) for i in wl_sensor)))
        ...
        # iterate through all ensembles of geometries
        for geo_ensemble in range(n_ensembles_geo):
            rest = crun_pergeo
            # iterate through all "splits" (max_per_file)
            for split in range(n_ensembles_split):
                ...
                # current run (first of the current split)
                run = geo_ensemble * crun_pergeo + split * max_per_file
                ...
                if prgbar_widget:
                    ...
                    prgbar_widget.gui.prgBar.setValue(
                        int(run * 100 / crun_max))
                    qgis_app.processEvents()
                ...
                save_array[npara:, :] = self.run_model(paras=dict(
                    zip(self.para_names, para_grid[run:run + nruns, :].T))).T
                ...
                np.save("{}_{:d}_{:d}".format(
                    LUT_dir + LUT_name, geo_ensemble, split), save_array)

        if prgbar_widget:
            prgbar_widget.gui.lblCaption_r.setText(
                'File {:d} of {:d}'.format(crun_max, crun_max))
