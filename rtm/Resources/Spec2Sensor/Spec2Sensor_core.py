        ...
        for sensor_band in range(self.n_wl_sensor):
            ...
            for srf_i in range(self.srf_nbands[sensor_band]):
                try:
                    ...
                    wlambda = int(
                        self.srf[srf_i][sensor_band][0] * int_factor_wl)
                except ValueError:
            ...
            sum_wfactor = sum(self.srf[i][sensor_band][1]
...
class BuildTrueSRF:
    ...
    def dframe_from_txt(self):
        ...
        sniffer = csv.Sniffer()
        # open just the first file and hope, they are all identical
        with open(self.files[0], 'r') as raw_file:
            # if the delimiter is not known before and passed to the class instance...
            ...
            try:
                # removes blank columns in K. Segl SRF files
                firstline = [i for i in next(raw) if i]
                secondline = [i for i in next(raw) if i]   # - " -
            except ImportError("Files could not be read."):
                return False

            # header is False by default (which means "not known")
        ...
        srf_list = list()
        # iterate through all the files (1 file = 1 target band)
        for i_file, single_file in enumerate(self.files):
            ...
            with open(single_file, 'r') as raw_file:
                raw = csv.reader(raw_file, delimiter=self.delimiter)
                for line in raw:
                    # all_lines contains all rows of data as items of a list
            ...
            if self.header_bool:
                ...
                if not len(header_items) == 2:
                    return False, "Error at file {}: Header has {:d} columns. Number of columns " \
                                  "expected is 2 (wavelengths and weights)".format(
                                      single_filename, len(header_items))
                skiprows = 1
            else:
                skiprows = 0
            ...
            try:
                ...
                wavelengths = [float(all_lines[i][0])
                               for i in range(skiprows, len(all_lines))]
                weights = [float(all_lines[i][1])
                           for i in range(skiprows, len(all_lines))]
            except ValueError:
            ...
            if not len(all_lines[1]) == 2:
                return False, "Error at file {}: Data has {:d} columns. Number of columns " \
                              "expected is 2 (wavelengths and weights)".format(
                                  single_filename, len(all_lines[1]))
    ...
    def srf_from_dframe(self, srf_list):
        ...
        # how many srf-values per band of the sensor?
        ...
        new_srf = np.full(shape=(np.max(srf_nbands), nbands_sensor, 2),
                          fill_value=self.nodat, dtype=np.float64)
        ...
        for band in range(nbands_sensor):
            new_srf[0:srf_nbands[band], band, 0] = np.asarray(
                srf_list[band][0][:srf_nbands[band]]) / self.wl_convert
            new_srf[0:srf_nbands[band], band, 1] = np.asarray(
                srf_list[band][1][:srf_nbands[band]])
        ...
        ndvi = list()
        # find band that is closest to "red"
        ndvi.append(np.argmin(np.abs(wavelength - 0.677)))
        # find band that is closest to "nir"
        ndvi.append(np.argmin(np.abs(wavelength - 0.837)))
        ...
        np.savez(self.out_file, srf_nbands=srf_nbands, srf=new_srf,
                 sensor_wl=wavelength, sensor_ndvi=ndvi)
        os.rename(self.out_file, os.path.splitext(self.out_file)
                  [0] + ".srf")  # rename from .npz to .srf
...
class BuildGenericSRF:
    ...
    def srf_from_imagery(self, x):   # fwhm-style
        ...
        for ix, x in enumerate(x_list):
            # if outreach of PDF is beyond PROSAIL range, then clip on both sides
            n_invalid = sum(i > 2500 or i < 400 for i in x)
            if n_invalid > 0:
        ...
        new_srf = np.full(shape=(np.max(new_srf_nbands), len(
            wavelength), 2), fill_value=self.nodat, dtype=np.float64)

        for wl in range(len(wavelength)):
            new_srf[0:new_srf_nbands[wl], wl, 0] = np.around(
                x_list[wl]/1000, decimals=3)
        ...
        np.savez(self.out_file, srf_nbands=new_srf_nbands,
                 srf=new_srf, sensor_wl=wavelength, sensor_ndvi=ndvi)
