...
class INFORM:
    ...
    def __init__(self, tts, tto, psi):
        ...
        self.tantts = torch.tan(tts)
        self.tantto = torch.tan(tto)
    ...
    def inform(self, cd, sd, h, sail_u, sail_inf, sail_tts_trans, sail_tto_trans):
        ...
        Fcd = co * cs + p * torch.pow((co * (1 - co) * cs * (1 - cs)), 0.5)
        # Tree crowns with sunlit background (Fcs)
        Fcs = co * (1 - cs) - p * \
            torch.pow((co * (1 - co) * cs * (1 - cs)), 0.5)
        ...
        G = Fcd.unsqueeze(1) * sail_tts_trans * sail_tto_trans + Fcs.unsqueeze(1) * sail_tto_trans + \
            Fod.unsqueeze(1) * sail_tts_trans + Fos.unsqueeze(1)

        C = Fcd.unsqueeze(1) * (1 - sail_tts_trans * sail_tto_trans)
