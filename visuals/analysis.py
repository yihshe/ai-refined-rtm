...
df3.rename(
    columns={f'output_{attr}': f'latent_{attr}' for attr in ATTRS[:-2]}, 
    inplace=True)
...
#     SAVE_PATH, 'histogram_realset_vars_nn_v_corr.png'), dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_PATH, 'scatter_realset_vars_pairs_wocorr.png'))
# %%
"""
...
# ATTRS2 = ATTRS[:-2]
ATTRS2 = ATTRS_VANILLA
df_coniferous = df0[df0['class'].isin(coniferous)]
df_deciduous = df0[df0['class'].isin(deciduous)]
for i, attr1 in enumerate(ATTRS2):
    for j, attr2 in enumerate(ATTRS2):
        ax = axs[i, j]
        sns.scatterplot(x=f'latent_{attr1}', y=f'latent_{attr2}',
...
plt.tight_layout()
plt.savefig(os.path.join(
    SAVE_PATH, 'scatter_realset_vars_pairs_vanilla_coniferous_v_deciduous.png'))
...
# axs[2, 3].axis('off')
plt.tight_layout()
plt.savefig(os.path.join(SAVE_PATH, 'linescatter_realset_bands_vanilla_target_v_output.png'))
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_PATH, 'timeseries_realset_vars_vanilla_coniferous_v_deciduous.png'))
plt.show()
