...
CSV_PATH = '/maps/ys611/ai-refined-rtm/saved/models/VanillaAE/0510_070335/model_best_loss_analyzer.csv'
SAVE_PATH = '/maps/ys611/ai-refined-rtm/saved/models/VanillaAE/0510_070335/histograms'
...
for band in S2_BANDS:
    ...
    # plt.tight_layout()
    # plt.savefig(os.path.join(SAVE_PATH, f'histogram_trainset_{band}.png'))
    # plt.show()
    plt.title(f'Test Data: Histogram of {band}')
    plt.xlabel('Squared Reconstruction Error')
    plt.ylabel('Frequency')
    plt.tight_layout()
    plt.savefig(os.path.join(SAVE_PATH, f'histogram_testset_loss_{band}.png'))
    plt.show()
...
for i, band in enumerate(S2_BANDS):
    ...
    # plt.tight_layout()
    # plt.savefig(os.path.join(SAVE_PATH, 'histogram_trainset.png'), dpi=300)
    # plt.show()
    sns.histplot(df[band].values, bins=NUM_BINS, ax=axs[i//4, i % 4])
    axs[i//4, i % 4].set_title(f'Histogram of {band}')
    ...
    axs[i//4, i % 4].set_ylabel('Frequency')
plt.tight_layout()
plt.savefig(os.path.join(SAVE_PATH, 'histogram_testset_loss.png'), dpi=300)
plt.show()
